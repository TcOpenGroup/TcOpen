...
public sealed class PushTcOpenGroupPackages : FrostingTask<BuildContext>
{
    ...
    {
        foreach (
            var nugetFile in Directory
                .EnumerateFiles(context.ArtifactsFolder, "*.nupkg")
    ...
    {
        {
            ...
            var doesExists = githubClient
                .Repository.Release.GetAll("TcOpenGroup", "TcOpen", ApiOptions.None)
                .Result.Any(p => p.Name == GitVersionInformation.SemVer);
            if (!doesExists)
            {

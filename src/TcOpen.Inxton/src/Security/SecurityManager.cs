...
namespace TcOpen.Inxton.Local.Security
{
    ...
    public class SecurityManager : ISecurityManager, ISecurityManagerUserInfo
    {
        ...
        private SecurityManager(IRepository<UserData> repository)
        {
            ...
            if (
                System.Threading.Thread.CurrentPrincipal?.GetType()
                != typeof(AppIdentity.AppPrincipal)
            )
            {
        ...
        private SecurityManager(
            ...
            IRepository<UserData> repository = null
        )
        {
            if (repository != null)
            {
        ...
        public static IAuthenticationService CreateDefault(string usersFolder = @"C:\INXTON\USERS\")
        {
            if (_manager == null)
            {
                _manager = new SecurityManager(
                    new DefaultUserDataRepository<UserData>(usersFolder)
                );
            }
            ...
            get
            {
                if (_manager == null)
                {
                    throw new SecurityManagerNotInitializedException(
                        "Security manager was not created."
                    );
                }
        ...
        public IEnumerable<string> AvailableGroups() =>
            availableRoles.Select(o => o.DefaultGroup).Distinct();
    ...
    public class SecurityManagerNotInitializedException : Exception
    {
        ...
        public SecurityManagerNotInitializedException(string message)
            : base(message) { }
        ...
        public SecurityManagerNotInitializedException(string message, Exception innerException)
        ...
        protected SecurityManagerNotInitializedException(

...
namespace TcOpen.Inxton.Local.Security.Readers
{
    ...
    public class ExternalTokenAuthorization : ExternalAuthorization
    {
        ...
        public ExternalTokenAuthorization(ITokenProvider tokenProvider, bool deauthenticateWhenSame)
        {
            ...
            this._tokenProvider.SetTokenReceivedAction(
                (token) => this.RequestAuthorization(token, deauthenticateWhenSame)
            );
        }
        ...
        public static ExternalAuthorization CreateComReader(
            string portName,
            int baudRate = 9600,
            int dataBits = 8,
        ...
        {
            return new ExternalTokenAuthorization(
                new ComPortTokenProvider(portName, baudRate, dataBits, stopBits, parity),
                deauthenticateWhenSame
            );
        }
        ...
        /// <returns></returns>
        public static ExternalAuthorization CreatePlcTokenReader(
            Vortex.Connector.ValueTypes.OnlinerString tokenValue,
            Vortex.Connector.ValueTypes.OnlinerBool tokenPresence,
            bool deauthenticateWhenSame = true
        )
        {
            return new ExternalTokenAuthorization(
                new PlcTokenReader(tokenValue, tokenPresence),
                deauthenticateWhenSame
            );
        }
    }

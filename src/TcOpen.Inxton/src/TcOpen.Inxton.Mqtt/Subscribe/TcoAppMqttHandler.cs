...
namespace TcOpen.Inxton.Mqtt
{
    ...
    {
        private readonly ConcurrentDictionary<string, IMqttApplicationMessageReceivedHandler> _subHandlers;
        public IEnumerable<string> TopicNames => _subHandlers.Keys;

        public TcoAppMqttHandler()
        {
            _subHandlers = new ConcurrentDictionary<string, IMqttApplicationMessageReceivedHandler>();
        }
        ...
        public async Task HandleApplicationMessageReceivedAsync(MqttApplicationMessageReceivedEventArgs eventArgs)
        {
            ...
            if (_subHandlers.ContainsKey(topic))
                await _subHandlers[eventArgs.ApplicationMessage.Topic].HandleApplicationMessageReceivedAsync(eventArgs);
        }

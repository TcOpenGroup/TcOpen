...
namespace TcOpen.Inxton.Mqtt
{
    public static class OnlinerBaseTypePublishExtensions
    {
        ...
        public static TcoMqttPrimitivePublisher<T> PublishChanges<T>(
            this OnlinerBaseType<T> onliner,
            IMqttClient client,
            string topic,
            PublishConditionDelegate<T> publishCondition = null
        )
        {
            var mqttWrapper = new TcoMqttPrimitivePublisher<T>(
                client,
                new OnlinerSerializer<T>(onliner)
            );

        ...
        public string AdvancedDiagnosticMessage
        {
            ...
            get
            {
                ...
                if (ErrorCodes.ErrorCode.ContainsKey(errorCode))
                     additionalInfo = ErrorCodes.ErrorCode.Where(key => key.Key == errorCode).FirstOrDefault().Value;
                else if ((errorCode>= 30001) && (errorCode <= 31901))
                    additionalInfo = ErrorCodes.ErrorCode.Where(key => key.Key == 30001).FirstOrDefault().Value;
    ...
    public static class ErrorCodes
    {
        ...
        static ErrorCodes()
        {
            ErrorCode = new Dictionary<int, string>()
            {
                ...
                {41016,"Program error: Out of target window, invalid force evaluation, position too low"},
                ...
                {41020,"Program error: Out of target window, invalid force evaluation, position exceeded"},
                {41021,"Program error: Out of target window, invalid position evaluation"},
                {41022,"Program error: Out of target window, force too low, invalid position evaluation"},
                {41023,"Program error: Out of target window, force exceeded, invalid position evaluation"},
                {41024,"Program error: Out of target window, invalid force and position evaluation"},

...
namespace Tc.Prober.Recorder
{
        ...
        where P : IPlain
    {
        ...
        public long CurrentFramePosition { get; private set; }
        ...
        {
            var frame = Frames
                .Where(p => p.Stamp >= CurrentFramePosition && p.Stamp <= lastFrameSector)
                .FirstOrDefault();
        ...
        {
            var frame = Frames
                .Where(p => p.Stamp >= position && p.Stamp <= lastFrameSector)
                .FirstOrDefault();
        ...
        public IList<RecordFrame<P>> Frames { get; } = new List<RecordFrame<P>>();
    }
}

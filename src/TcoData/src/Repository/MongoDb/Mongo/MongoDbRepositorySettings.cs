...
namespace TcOpen.Inxton.Data.MongoDb
{
    ...
    public class MongoDbRepositorySettings<T> : RepositorySettings
        where T : IBrowsableDataObject
    {
        ...
        private IMongoCollection<T> GetCollection(string collectionName)
        {
            ...
            var existingClient = Collections
                .Where(p => p.Key == collectionName)
                .Select(p => p.Value);
            if (existingClient.Count() >= 1)
            {
                return existingClient.ElementAt(0);
        ...
        private static Dictionary<string, IMongoClient> Clients =
            new Dictionary<string, IMongoClient>();
        private static Dictionary<string, IMongoDatabase> Databases =
            new Dictionary<string, IMongoDatabase>();
        private static Dictionary<string, IMongoCollection<T>> Collections =
        ...
        {
            if (
                BsonClassMap.GetRegisteredClassMaps().FirstOrDefault(p => p.ClassType == typeof(T))
                == null
            )
            {
            ...
            {
                var existing = BsonClassMap
                    .GetRegisteredClassMaps()
                    .FirstOrDefault(p => p.ClassType == type);
                if (existing == null)
                {
            ...
            try
            {
                BsonSerializer.RegisterSerializer(
                    typeof(UInt64),
                    new UInt64Serializer(BsonType.Int64, new RepresentationConverter(true, false))
                );
                BsonSerializer.RegisterSerializer(
                    typeof(UInt32),
                    new UInt32Serializer(BsonType.Int64, new RepresentationConverter(true, false))
                );
        ...
        {
            Console.WriteLine(
                $"Checking that mognodb server at '{Client.Settings.Server.ToString()}' is running."
            );
            ...
            while (true)
            {
                if (
                    IsPortOpen(
                        Client.Settings.Server.Host,
                        Client.Settings.Server.Port,
                        new TimeSpan(10000)
                    )
                )
                {
            ...
            Console.WriteLine(
                $"Connection to server '{Client.Settings.Server.ToString()}' is available."
            );
        }

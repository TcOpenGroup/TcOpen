        ...
        public object Convert(object ovalue, Type targetType, object parameter, CultureInfo culture)
        {
            ...
            if (timeSpan.Hours > 0)
            {
                sb.AppendFormat(
                    ...
                    timeSpan.Hours,
                    (timeSpan.Hours > 1) ? localized.Hours : localized.Hour
                );
                if (approximate)
                    return sb.ToString() + suffix;
            }
            if (timeSpan.Minutes > 0)
            {
                sb.AppendFormat(
                    ...
                    (sb.Length > 0) ? ", " : string.Empty,
                    timeSpan.Minutes,
                    (timeSpan.Minutes > 1) ? localized.Minutes : localized.Minute
                );
                if (approximate)
                    return sb.ToString() + suffix;
            }
            if (timeSpan.Seconds > 0)
            {
                sb.AppendFormat(
                    ...
                    (sb.Length > 0) ? ", " : string.Empty,
                    timeSpan.Seconds,
                    (timeSpan.Seconds > 1) ? localized.Seconds : localized.Second
                );
                if (approximate)
                    return sb.ToString() + suffix;
            }
            if (sb.Length == 0)
                return localized.RightNow;

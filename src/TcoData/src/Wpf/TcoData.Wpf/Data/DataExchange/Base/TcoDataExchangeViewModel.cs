...
namespace TcoData
{
    public class TcoDataExchangeViewModel : RenderableViewModel
    {
        ...
        public dynamic DataViewModel { get; private set; }
        ...
        protected virtual void UpdateAvailability()
        {
            try
            {
                ...
                ((FunctionAvailability)this.DataViewModel).StartCreateCopyOfExistingAvailable =
                    true;
        ...
        private void CreateBrowsable(TcoDataExchange dataExchangeObject)
        {
            if (!PresentationHelper.IsDesignMode())
            {
                try
                {
                    ...
                    {
                        DataPropertyInfo = dataExchangeObject
                            .GetType()
                            .GetProperty("_data", BindingFlags.NonPublic);
                    }
                    ...
                    if (DataPropertyInfo == null)
                    {
                        DataPropertyInfo = dataExchangeObject
                            .GetType()
                            .GetProperty("_data", BindingFlags.NonPublic | BindingFlags.Instance);
                    }

                    if (DataPropertyInfo == null)
                    {
                        throw new Exception(
                            $"{dataExchangeObject.GetType().ToString()} must implement member '_data' that inherits from {nameof(TcoEntity)}."
                        );
                    }
                    ...
                    var genericTypeName =
                        $"{dataNameSpace}.Plain{dataOfType}, {dataNameSpace}Connector";
                    var genericType = Type.GetType(genericTypeName);

                    if (genericType == null)
                    {
                        throw new Exception(
                            $"Could not retrieve {genericTypeName} when creating browsable object."
                        );
                    }

                    MethodInfo generic = method.MakeGenericMethod(genericType);

                    DataViewModel = generic.Invoke(
                        null,
                        new object[] { dataExchangeObject.GetRepository(), dataExchangeObject }
                    );
                }
                catch (Exception ex)
                {
                    throw new BrowsableObjectCreationException(
                        "Unable to create browsable object for the view. For details see inner exception.",
                        ex
                    );
                }
            }

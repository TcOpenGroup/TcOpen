...
namespace Sandbox.IntegrationProjects.Wpf
{
    public class MainWindowViewModel : BindableBase
    {
        ...
        {
            SwiftRecordVm = new SwiftRecorderViewModel(
                IntegrationProjectsPlc.MAIN_TECHNOLOGY._technology._ST001
            );

            IntegrationProjectsPlc.MAIN_TECHNOLOGY._technology._ST001._serviceModeTask.Execute(
                null
            );
            System.Threading.Thread.Sleep(1000);
            IntegrationProjectsPlc.MAIN_TECHNOLOGY._technology._ST001._groundSequenceTask.Execute(
                null
            );
            var client = CreateClientAndConnect();

            IntegrationProjectsPlc.MAIN_TECHNOLOGY._technology._startCycleCount.PublishChanges(
                client,
                "fun_with_TcOpen_Hammer",
                publishCondition: (lastPublished, toPublish) => true
            );

            IntegrationProjectsPlc.MAIN_TECHNOLOGY._technology._mirrorStartCycle.Subscribe(
                client,
                "fun_with_TcOpen_Hammer"
            );
        ...
        private static IMqttClient CreateClientAndConnect()
        {
            ...
            var c = MqttFactory
                .CreateMqttClient()
                .UseApplicationMessageReceivedHandler(new TcoAppMqttHandler());
        ...
        public IntegrationProjectsTwinController IntegrationProjectsPlc { get; } =
            Entry.IntegrationProjectsPlc;
    }
}

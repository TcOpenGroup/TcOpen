...
namespace TcoCoreUnitTests.PlcTcRpc
{
    public class T04_TcoStateTests
    {
        ...
        public void T402_ChangeState()
        {
            ...
            newState = TestHelpers.RandomNumber(
                (short)(initState + 1),
                (short)(5 * (initState + 1))
            ); //Generate new random value of the new state.
            ...
            Assert.AreEqual(
                "My state has been change from "
                    + initState.ToString() //Check if overiden method OnStateChange() generate expected message.
                    + " to the new state "
                    + newState.ToString()
                    + ".",
                ts.GetMessage()
        ...
        public void T403_OnStateChange()
        {
            ...
            Assert.AreEqual(
                "My state has been change from "
                    + initState.ToString() //Check if overiden method OnStateChange() generate expected message and overwrite the message randomly generated.
                    + " to the new state "
                    + newState.ToString()
                    + ".",
                ts.GetMessage()
        ...
        public void T407_AutoRestoreOnStateChange()
        {
            ...
            tc.RunUntilEndConditionIsMet(
                ...
                () => tis_a >= 10 && tis_b >= 10
            ); //End condition- both task are in state 10 or greather

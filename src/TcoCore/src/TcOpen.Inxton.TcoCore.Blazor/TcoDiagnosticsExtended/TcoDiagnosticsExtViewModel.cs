...
namespace TcoCore
{
    public class TcoDiagnosticsExtViewModel : RenderableViewModelBase
   
        {
            ...
            public void AcknowledgeMessage(PlainTcoMessage msg)
            {
                if (msg != null)
                {
        ...
        public void AcknowledgeAllMessages()
        {
            lock (updatemutex)
            {
                try
                {
                    DateTime currentTimestamp = DateTime.Now; // Get the current timestamp

                    TcoAppDomain.Current.Logger.Information("All messages acknowledged {@payload}",
                        new { rootObject = _tcoObject.HumanReadable, rootSymbol = _tcoObject.Symbol, timestamp = currentTimestamp });

                    foreach (var item in MessageDisplay.Where(p => p.Pinned))
                    {
                        item.TimeStampAcknowledged = currentTimestamp; // Set the TimeStampAcknowledged
                        item.OnlinerMessage.Pinned.Cyclic = false;

                        TcoAppDomain.Current.Logger.Information("Message acknowledged {@message} at {timestamp}",
                            new { Text = item.Text, Category = item.CategoryAsEnum, timestamp = currentTimestamp });
                    }
                }
                catch (Exception ex)
                {

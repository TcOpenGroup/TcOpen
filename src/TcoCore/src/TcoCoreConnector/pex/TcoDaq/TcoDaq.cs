...
namespace TcoCore
{
    public partial class TcoDaq
    {
        ...
        {        
            expectDequeingTags = this.Buffer.Select(p => p.ExpectDequeing as IValueTag).ToList();            
        }
        ...
        public void StartLoggingMessages<T>(eMessageCategory minLevelCategory, int interLoopDelay = 25, ILogger logTarget = null) where T : TcoDaqItem
        {                        
            Task.Run(() =>
            {
                while (true)
        ...
        public IEnumerable<T> Pop<T>() where T : TcoDaqItem
        {
        ...
        public void LogMessages<T>(IEnumerable<T> messages)
        {          

...
namespace Grafana.Backend.Queries
{
    public class OkNokByRecipe : IQuery<PlainStation001_ProductionData>
    {
        public ITable Query(IQueryable<PlainStation001_ProductionData> production, DateTime from, DateTime to) => production
            ...
            .GroupBy(x => x.Recipe)
            .Select(x => new { Recipe = x.Key, OK = x.Where(x => x.Result).Count(), NOK = x.Where(x => !x.Result).Count() })
            .OrderBy(x=> x.Recipe)
            .ToTable()
            ;
    }
    ...
    {
        public ITable Query(IQueryable<PlainStation001_ProductionData> production, DateTime from, DateTime to)
        {
        ...
        class Result { public int Good { get; set; } public int Bad { get; set; } };

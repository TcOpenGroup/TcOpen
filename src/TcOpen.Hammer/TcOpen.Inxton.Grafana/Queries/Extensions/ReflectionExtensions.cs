...
namespace Grafana.Backend.Queries.Extensions
{
    ...
    {
        private static IEnumerable<(string Path, T Object)> PlainChildrenOf<T>(object root, string parentName, List<T> acc)
        {
        ...
        private static string NameOfObject(object parent, object @object) => parent
           .GetType()
           .GetProperties()
           .First(x => x.GetValue(parent) == @object)
           .Name;

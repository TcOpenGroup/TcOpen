...
namespace TcOpenHammer.Grafana.API
{
    ...
    public class ProcessDataService : IQueryableService
    {
        ...
        private readonly IDictionary<string, IQuery<PlainStation001_ProductionData>> ProcessDataQueres;

        public ProcessDataService(MongoService databaseService)
        {
            _databaseService = databaseService;
            ProcessDataQueres = new Dictionary<string, IQuery<PlainStation001_ProductionData>>()
        ...
        public ITable ExecuteQuery(QueryRequest request, string target) => ProcessDataQueres[target]
            .Query(

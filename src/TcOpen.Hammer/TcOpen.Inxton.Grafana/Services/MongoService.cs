...
namespace TcOpenHammer.Grafana.API.Model.Mongo
{
    ...
    public class MongoService
    {
        ...
        public MongoService(DatabaseSettings settings, ILogger<MongoService> logger)
        {
            ...
            ProcessData = database.GetCollection<PlainStation001_ProductionData>(
                settings.ProductionCollectionName
            );
            StationModes = database.GetCollection<enumModesObservedValue>(
                settings.StationModesCollection
            );
            RecipeHistory = database.GetCollection<ObservedValue<string>>(
                settings.Station001RecipeHistoryCollection
            );
        }
        ...
        private void SetupSerialisationAndMapping()
        {
            if (
                BsonClassMap
                    ...
                    .FirstOrDefault(p => p.ClassType == typeof(PlainStation001_ProductionData))
                == null
            )
            {
